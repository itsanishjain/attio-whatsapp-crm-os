...
async function syncMessageToAttio(input: {
  ...
  const messageText =
    message.textBody ?? (message.hasMedia ? 'Media message' : null);

  await client.updatePerson(recordId, {
    whatsapp_last_message_at: new Date(message.sentAt).toISOString(),
    whatsapp_last_message_direction:
      message.direction === 'outbound' ? 'Outbound' : 'Inbound',
    ...(messageText && !metadataOnly
      ? { whatsapp_last_message_text: messageText }
      : {}),
    ...(!isGroup && contactPhone
      ? {
          whatsapp_conversation_link: `https://wa.me/${contactPhone.replace(/\D/g, '')}`,
        }
      : {}),
  });
